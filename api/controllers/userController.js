...
const createUser = async (req, res) => {
  ...
  try {
    ...
    const newUser = await connection('usuario_nuevo')
      .insert(user)
      .returning('*');
...
const login = async (req, res) => {
  ...
  try {
    const user = await connection('usuario_nuevo').first('*').where({
      email,
    });
...
/* 
    ...
    Authorization: Token obtenido por el login
